function getHostUrl(): string {
  const hostUrl =
    process.env.HOST_URL || (process.env.NODE_ENV === 'development' && 'http://localhost:3005')
...
const config = Object.freeze({
  mongoConnectString:
    ...
    (process.env.NODE_ENV === 'development' && 'mongodb://localhost:27017/kodi'),

  sessionSecret:
    process.env.SESSION_SECRET || (process.env.NODE_ENV === 'development' && 'TopSecret'),
