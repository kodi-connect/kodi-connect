...
const anotherEvent = {
  event: {
    directive: {
      header: {
        namespace: 'Alexa.Authorization', name: 'AcceptGrant', messageId: '9e62f970-c490-4817-b1fc-8cbe2842017f', payloadVersion: '3',
      },
      payload: {
...
describe('Authorization', () => {
  test('should authorize', async () => {
    const getUserAuthTokensSpy = jest.spyOn(amazon, 'getUserAuthTokens').mockImplementation(() => ({ token: 'abc' }));
    const storeAmazonTokensSpy = jest.spyOn(users, 'storeAmazonTokens').mockImplementation(() => null);
