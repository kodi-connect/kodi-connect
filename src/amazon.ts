...
import { AwsRegion, AccessTokenRequest, AmazonTokens } from './types'
...
export async function getUserAuthTokens(
  username: string,
  region: AwsRegion,
  code: string
): Promise<AmazonTokens> {
  const alexaSkillMessagingCredentials = await getAlexaSkillMessagingCredentials(username)
  return accessTokenRequest(alexaSkillMessagingCredentials, region, {
...
export async function getRegionAndAccessToken(
  username: string
): Promise<{ region: AwsRegion; accessToken: string } | null | undefined> {
  const currentTime = Date.now()
  ...
  if (currentTime >= amazonTokens.expires_at + 10 * 1000) {
    amazonTokens = await refreshAccessToken(
      username,
      amazonTokens.region,
      amazonTokens.refresh_token
    )
