...
export default function createTemplatesRouter(kodiInstances: KodiInstances) {
  ...
  router.get('/login', isLoggedInMiddleware(false), wrapAsync(async (req, res) => {
    logger.info('LOGIN', req.query);
    res.render('login', { ...req.query, error: _.get(res, 'locals.app.errorMessage') });
  }));

  router.get('/register', isLoggedInMiddleware(false), wrapAsync(async (req, res) => {
    res.render('register', { error: _.get(res, 'locals.app.errorMessage') });
  }));

  router.get('/confirm/:confirmationToken', wrapAsync(async (req, res) => {
  ...
  router.get('/devices', isLoggedInMiddleware(true), wrapAsync(async (req, res) => {
    const devices = await getDevices(req.session.user.username);
    const devicesWithStatus = devices.map((device) => ({ ...device, connected: kodiInstances[device.id] != null }));

    res.render('devices', { devices: devicesWithStatus, error: _.get(res, 'locals.app.errorMessage') });
  }));

  router.get('/terms-of-use/alexa', wrapAsync(async (req, res) => {
    res.render('alexa-terms-of-use', { hostname: req.hostname });
  }));

  router.get('/privacy-policy/alexa', wrapAsync(async (req, res) => {
    res.render('alexa-privacy-policy', { hostname: req.hostname });
  }));

  router.get('/admin', isAdminMiddleware, wrapAsync(async (req, res) => {
